...
namespace DDG
{
   ...
   int Mesh::read(const string& filename, bool postProcessing)
   {
      ...
      if( !in.is_open() )
      {
          qDebug() << "Error reading from mesh file " << QString::fromStdString(filename) << endl;
   ...
   // only if there was an error
   {
      ...
      if( !out.is_open() )
      {
         qDebug() << "Error writing to mesh file " << QString::fromStdString(filename) << endl;
   ...
   void Mesh :: initializeSmoothStructure( void )
   {
                      ...
                      v ++ )
      {
         ...
         // find the first he if v in on boundary
         if (v->onBoundary()) {
             // check the first he
         ...
         // Ryan added
             ...
             do
             {
                 he->angularCoordinate *= M_PI / cumulativeAngle;
                 he = he->flip->next;
             } while (he != v->he && (!he->onBoundary));
         }
      }
   ...
   void Mesh :: computeTrivialSection( void )
   {
      ...
      if( indexSum != chi )
      {
         ...
         qDebug() << "WARNING: singularity indices do not satisfy Poincaré-Hopf!" << endl;
   ...
   {
       qDebug() << "Computing curvature-aligned direction field..." << endl;
   ...
   {
       qDebug()<<"Computing stripe pattern..." << endl;
   ...
   // the eigenvalue problem.
   {
      ...
      for( FaceIter f = faces.begin(); f != faces.end(); f++ )
      {
         ...
         f->paramIndex[p] = n; //mark down // tianyu 20240602
      }
   }
