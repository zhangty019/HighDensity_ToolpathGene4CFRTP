...
bool QMeshPatch::inputStripOBJFile(char* filename) {
	...
	while (!feof(fp)) {
		...
		if ((strlen(buf) == 1) && (buf[0] == 'v'))
		{
			...
			node->SetCoord3D(xx, yy, zz);
			node->SetCoord3D_last(xx, yy, zz);
			node->SetIndexNo(nodeList.GetCount() + 1);//start from 1
			node->selected = false;
			nodeList.AddTail(node);
		}
	}
	...
	while (!feof(fp)) {
		...
		if ((strlen(buf) == 1) && (buf[0] == 'l'))
		{
			sscanf(linebuf, "%s %d %d\n", buf, &ls, &le);

			startNode = nodeArray[ls - 1];
			endNode = nodeArray[le - 1];
			edge = new QMeshEdge;
			edge->SetMeshPatchPtr(this);
			edge->SetStartPoint(startNode);
			edge->SetEndPoint(endNode);
			...
			(endNode->GetEdgeList()).AddTail(edge);
			edge->CalLength();
			edge->SetIndexNo(edgeList.GetCount() + 1);//start from 1
			edgeList.AddTail(edge);
	...
	int edgeNum = edgeList.GetCount();
	edgeArray = new QMeshEdge * [edgeNum];
	i = 0;
	for (Pos = edgeList.GetHeadPosition(); Pos != NULL; i++) {
		edge = (QMeshEdge*)(edgeList.GetNext(Pos));
		edgeArray[i] = edge;
	}
	...
	while (fgets(linebuf, sizeof(linebuf), fp) != NULL) {
		if (sscanf(linebuf, "%s %d", buf, &f_id) == 2 && strcmp(buf, "#li") == 0) {

			edgeArray[i]->attached_face_idx = f_id;
			i++;
		}
	}

	//std::cout <<"edgeList number: "<< edgeList.GetCount() << " attached face number: "<< i << std::endl;
