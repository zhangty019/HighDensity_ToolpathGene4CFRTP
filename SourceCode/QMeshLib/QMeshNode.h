...
public:
	...
	Eigen::Vector3d directionField = Eigen::Vector3d::Zero(); //(all = b + s)

	QMeshEdge* relatedLayerEdge = NULL; //for tool-path generation, iso-Node on which layer edge
	bool isBoundaryNode = false;

	//mesh offset
	int nodeInd_ofOffsetNode_onThisNode = -1; 

	//used for strip path generation
	bool is_endPnt = false;
	int seg_Idx = -1;
	bool is_shortChain = false;
	bool is_closeLoop = false;
	...
	//add for toolpath tangent direction
	Eigen::Vector3d m_printTan = Eigen::Vector3d::Zero();
private:
